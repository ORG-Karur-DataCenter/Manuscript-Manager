...
const TRANSPORTS = {
  ...
  async meta(recipient, message, { env, fetchImpl }) {
    ...
    /*
     * A test sends the REAL template, not Meta's hello_world.
     *
     * It used to send hello_world, because the custom template was still
     * awaiting approval and the point was to prove the connection existed at
     * all. That reason expired the day the template was approved, and what was
     * left was a test that exercised a message the app never sends: a
     * different template, in a different language code.
     *
     * It then failed while real reminders were being delivered to the same
     * person, on the same token and the same sender, and was believed --
     * costing an evening of chasing a phone number that was never the problem.
     * A test that can fail while the thing it tests works is worse than no
     * test, because it is trusted.
     *
     * So a test now differs from a reminder in one way only: the words. If
     * this arrives, a real reminder would have arrived.
     *
     * hello_world remains the fallback for the case it was written for --
     * no custom template configured yet, nothing else that could be sent.
     */
    const isTest =
      String(env.META_WHATSAPP_TEST || "").toLowerCase() === "true";

    const usingFallback = isTest && !template;
    const templateName = usingFallback ? "hello_world" : template;

    const languageCode = usingFallback
      ? "en_US"
    ...
    const payload = templateName
      ? {
          ...
          template: {
            ...
            /*
             * hello_world takes no parameters; the real template needs all
             * five, test or not -- Meta rejects a template sent with the wrong
             * number of them, which is itself worth finding in a test.
             */
            ...(usingFallback
              ? {}
              : {
